import { PassportStoreState, initialState } from '@hooks/data/passport/store';
...
const setup = (initStateOverride: Partial<PassportStoreState> = {}) => {
  const mockState = (): PassportStoreState => ({
    ...initialState,
    ...initStateOverride,
  });

  const mockUseStore = create<PassportStoreState>()(queryParamMiddleware(mockState));
  return { mockUseStore };
};
