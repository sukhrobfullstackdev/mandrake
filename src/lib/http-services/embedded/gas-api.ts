import { GAS_API_URL } from '@constants/env';
...
export class GasHttpService extends HttpServiceAbstract {
  protected static BASE_URL = GAS_API_URL;

  protected SERVICE_NAME = 'GAS';

  async Get(route: string, headers?: HeadersInit) {
    const response = await fetch(GasHttpService.BASE_URL + route, {
      method: 'GET',
  ...
  protected getHeaders(headers?: HeadersInit) {
    ...
    return {
      ...
      'x-magic-auth-user-id': useStore.getState().authUserId as string,
