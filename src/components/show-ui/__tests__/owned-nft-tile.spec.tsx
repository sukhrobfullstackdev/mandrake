...
const MOCK_OWNED_NFT: any = {
  contract: {
    ...
    openSeaMetadata: {
      ...
      externalUrl: 'https://magic.link',
      imageUrl: 'https://bafybeia2y4jr6hbccb5rtnnbhieu3rvzz6vbodycx7l4bi2a7qy4txml3i.ipfs.dweb.link',
...
function setup() {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <OwnedNftTile nft={MOCK_OWNED_NFT} balance={Number(MOCK_OWNED_NFT.balance)} />
    </QueryClientProvider>,
  );
}

describe('Owned Nft Tile', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });

  it('should display the image with alt text "Magic x KBW ', () => {
    setup();
  ...
  it('should display balance of 6', () => {
    setup();

    const content = screen.getByText('x6');
    expect(content).toBeInTheDocument();
  });
});
