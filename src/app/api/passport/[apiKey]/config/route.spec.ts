...
describe('success', () => {
  it('should return data with status 200 if passport config response is successful', async () => {
    ...
    jest
      .spyOn(HttpService.PassportIdentity, 'Get')
...
describe('failure', () => {
  it('should return with status 400 if no apiKey provided', async () => {
    ...
    expect(message).toBe(NextResponseErrorMessage.PassportConfigCacheKeyMissing);
    expect(status).toBe('failed');
  });

  it('should return with status 400 if passport config response is empty or unexpected', async () => {
    jest.spyOn(console, 'error').mockImplementation(jest.fn()); // expected to be called in the handler
    jest.spyOn(console, 'warn').mockImplementation(jest.fn()); // expected to be called in createNextErrorResponse()
    ...
    jest
      .spyOn(HttpService.PassportIdentity, 'Get')
      .mockImplementation(jest.fn(() => new Promise(res => res(undefined))));
    ...
    expect(message).toBe(NextResponseErrorMessage.PassportConfigCacheDataMissing);
    expect(status).toBe('failed');
  });

  it('should return with status 500 and surface error if passport service called fails', async () => {
    jest.spyOn(console, 'error').mockImplementation(jest.fn()); // expected to be called in the handler
    jest.spyOn(console, 'warn').mockImplementation(jest.fn()); // expected to be called in createNextErrorResponse()

    // mock passport service request to return undefined
    jest.spyOn(HttpService.PassportIdentity, 'Get').mockImplementation(
      jest.fn(() => {
        throw new Error();
      }),
    );
