...
export default function SendNativeGasTokens() {
  ...
  const {
    smartAccount,
    isPreparingAccount,
    // TODO: need to account for case where cab just isn't enabled for this chain, and think about CEX eligibility cases too
    // isCabEnabledOnCurrentChain,
  } = useSmartAccount();
  ...
  const doSendNativeGasToken = async () => {
    if (!calls) throw new Error('doSendNativeGasToken: No calls found in payload');
    if (!smartAccount) throw new Error('doSendNativeGasToken: Smart account not found');
    if (!network) throw new Error('doSendNativeGasToken: Network not found');

    setHasTransactionBeenSent(true);
    const userOpCalls: KernelClientCallData[] = calls.map(c => ({
      to: c.to,
      value: BigInt(c.value || 0),
      data: formatDataField(c),
    }));
    ...
    try {
      ...
      logger.error('Error sending transaction - send_native_gas_tokens', error);
    ...
    return (
      <PassportTransactionStatus
        ...
        onTransactionError={error => {
          logger.error('Error completing transaction - send_native_gas_tokens', error);
          handleError(error as Error);
        }}
      />
    );
  ...
  return (
    <PassportPage>
      ...
      <TransactionConfirmButton
        ...
        hasEnoughForFee={hasEnoughForFee}
        hasEnoughForSendAmount={hasEnoughForSendAmount}
        onConfirm={doSendNativeGasToken}
      />
