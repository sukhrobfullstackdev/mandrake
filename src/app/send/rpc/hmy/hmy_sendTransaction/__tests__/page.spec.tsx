import Page from '@app/send/rpc/hmy/hmy_sendTransaction/page';
...
const mockBridge = { sendTransaction: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { sendTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated harmony signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.sendTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
      expect(mockResolve).toHaveBeenCalledWith('signature');
      expect(mockReject).not.toHaveBeenCalled();
    });
  });

  it('user logged in: should call rejectActiveRpcRequest if harmony bridge throws error', async () => {
    (DkmsService.reconstructSecret as jest.Mock).mockResolvedValue('decryptedPrivateKey');
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockBridgeError }));
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.sendTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
