...
export default function HarmonyGetBalancePage() {
  ...
  const doHarmonyGetBalanceForUser = async () => {
    ...
    try {
      ...
      const res = await harmonyBridge.getBalance(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'hmy_getBalance Page - Error hydrating or creating multichain wallet',
