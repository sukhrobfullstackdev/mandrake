import Page from '@app/send/rpc/hmy/hmy_getBalance/page';
...
const mockBridge = { getBalance: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { getBalance: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated harmony signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.getBalance).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if harmony bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.getBalance).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
