import Page from '@app/send/rpc/kda/kda_signTransaction/page';
...
const mockKadenaBridge = { signTransaction: jest.fn().mockResolvedValue('signature') };
const mockKadenaBridgeError = { signTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Kda Sign Transaction', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated Kadena signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockKadenaBridge.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if Kadena bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockKadenaBridgeError.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
