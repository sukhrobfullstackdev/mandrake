...
export default function KdaSignTransactionWithSpireKeyPage() {
  ...
  const { walletInfoData, credentialsData } = useHydrateOrCreateMultichainWallet({
    enabled: !!authUserId && !!authUserSessionToken,
    walletType: WalletType.KADENA,
  });

  const kdaGetAccount = () => walletInfoData?.publicAddress || '';
  const rejectActiveRpcRequest = useRejectActiveRpcRequest();
  const resolveActiveRpcRequest = useResolveActiveRpcRequest();

  const doKdaSendTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await kadenaBridge.signTransactionWithSpireKey(activeRpcPayload);
      resolveActiveRpcRequest(JSON.parse(JSON.stringify(res)));
    } catch (err) {
