import Page from '@app/send/rpc/tezos/tezos_sendContractInvocationOperation/page';
...
const mockBridge = { sendContractInvocation: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { sendContractInvocation: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.sendContractInvocation).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.sendContractInvocation).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
