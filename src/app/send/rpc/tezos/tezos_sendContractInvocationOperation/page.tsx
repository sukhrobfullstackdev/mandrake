...
export default function TezosSendContractInvocationPage() {
  ...
  const doTezosSendContractInvocationForUser = async () => {
    ...
    try {
      ...
      const res = await tezosBridge.sendContractInvocation(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doTezosSendContractInvocationForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'tezos_sendContractInvocationOperation Page - Error hydrating or creating multichain wallet',
