...
export default function TezosSendTransactionPage() {
  ...
  const doTezosSendTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await tezosBridge.sendTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'tezos_sendTransaction Page - Error hydrating or creating multichain wallet',
