...
export default function TezosGetAccountPage() {
  ...
  const doTezosGetAccountForUser = async () => {
    ...
    try {
      ...
      const res = await tezosBridge.getAccount(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'tezos_getAccount Page - Error hydrating or creating multichain wallet',
