...
export default function TezosSendTransactionPage() {
  ...
  const doTezosSendContractPingOperationForUser = async () => {
    ...
    try {
      ...
      const res = await tezosBridge.sendContractPingOperation(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doTezosSendContractPingOperationForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'tezos_sendContractPing Page - Error hydrating or creating multichain wallet',
