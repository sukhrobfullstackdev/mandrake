...
export default function TezosSendContractOriginationPage() {
  ...
  const doTezosSendContractOriginationForUser = async () => {
    ...
    try {
      ...
      const res = await tezosBridge.sendContractOrigination(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doTezosSendContractOriginationForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'tezos_sendContractOriginationOperation Page - Error hydrating or creating multichain wallet',
