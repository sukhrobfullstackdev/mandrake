import Page from '@app/send/rpc/tezos/tezos_sendContractOriginationOperation/page';
...
const mockBridge = { sendContractOrigination: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { sendContractOrigination: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.sendContractOrigination).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.sendContractOrigination).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
