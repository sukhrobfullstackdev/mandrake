import Page from '@app/send/rpc/tezos/tezos_sendDelegationOperation/page';
...
const mockBridge = { sendDelegation: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { sendDelegation: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.sendDelegation).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.sendDelegation).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
