import Page from '@app/send/rpc/hedera/hedera_sign/page';
...
const mockHederaBridge = { sign: jest.fn().mockResolvedValue('signature') };
const mockHederaBridgeError = { sign: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated hedera signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockHederaBridge.sign).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if hedera bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockHederaBridgeError.sign).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
