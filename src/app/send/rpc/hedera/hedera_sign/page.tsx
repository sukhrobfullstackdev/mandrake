...
export default function HederaSignPage() {
  ...
  const doHederaSignForUser = async () => {
    ...
    try {
      ...
      const res = await hederaBridge.sign(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error('hedera_sign Page - Error hydrating or creating multichain wallet', multichainWalletHydrationError);
    }
