...
import { HederaLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function HederaGetPublicKeyPage() {
  ...
  const hederaGetAccount = () => walletInfoData?.publicAddress || '';
  const rejectActiveRpcRequest = useRejectActiveRpcRequest();
  const resolveActiveRpcRequest = useResolveActiveRpcRequest();
  const doHederaGetPublicKeyForUser = async () => {
    ...
    try {
      ...
      const hederaBridge = (await createBridge(hederaGetAccount, ethNetwork, ext)).ledgerBridge as HederaLedgerBridge;
      const res = await hederaBridge.getPublicKey(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'hedera_getPublicKey Page - Error hydrating or creating multichain wallet',
