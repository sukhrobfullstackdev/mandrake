import Page from '@app/send/rpc/hedera/hedera_getPublicKey/page';
...
const mockHederaBridge = { getPublicKey: jest.fn().mockResolvedValue('signature') };
const mockHederaBridgeError = { getPublicKey: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  beforeEach(() => {
    ...
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockHederaBridge }));
    (AtomicRpcPayloadService.getActiveRpcPayload as jest.Mock).mockReturnValue(mockActiveRpcPayload);
  });
  afterEach(() => jest.clearAllMocks());

  it('user logged in: should call resolveActiveRpcRequest with successfuly generated hedera signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockHederaBridge.getPublicKey).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
      expect(mockResolve).toHaveBeenCalledWith('signature');
      expect(mockReject).not.toHaveBeenCalled();
    });
  });

  it('user logged in: should call rejectActiveRpcRequest if hedera bridge throws error', async () => {
    (DkmsService.reconstructSecret as jest.Mock).mockResolvedValue('decryptedPrivateKey');
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockHederaBridgeError }));
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockHederaBridgeError.getPublicKey).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
