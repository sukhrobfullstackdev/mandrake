import Page from '@app/send/rpc/algod/algod_signGroupTransactionV2/page';
...
const mockBridge = { signGroupTransactionV2: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { signGroupTransactionV2: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.signGroupTransactionV2).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.signGroupTransactionV2).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
