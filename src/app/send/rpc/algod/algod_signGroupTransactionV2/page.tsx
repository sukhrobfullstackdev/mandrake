...
export default function AlgodSignGroupTransactionV2Page() {
  ...
  const doAlgodSignGroupTransactionV2ForUser = async () => {
    ...
    try {
      ...
      const res = await algorandBridge.signGroupTransactionV2(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doAlgodSignGroupTransactionV2ForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'algod_signGroupTransactionV2 Page - Error hydrating or creating multichain wallet',
