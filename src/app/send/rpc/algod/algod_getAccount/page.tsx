...
export default function AlgodGetAccountPage() {
  ...
  const doAlgodGetAccountForUser = async () => {
    ...
    try {
      ...
      const res = await algorandBridge.getAccount(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'algod_getAccount Page - Error hydrating or creating multichain wallet',
