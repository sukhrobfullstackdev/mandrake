...
export default function AlgodSignTransactionPage() {
  ...
  const doAlgodSignTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await algorandBridge.signTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'algod_signTransaction Page - Error hydrating or creating multichain wallet',
