...
export default function AlgodSignGroupTransactionPage() {
  ...
  const doAlgodSignGroupTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await algorandBridge.signGroupTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doAlgodSignGroupTransactionForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'algod_signGroupTransaction Page - Error hydrating or creating multichain wallet',
