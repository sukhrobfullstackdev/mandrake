...
export default function Page() {
  ...
  const { didToken, error: didTokenError } = useCreateDidTokenForUser({
    ...
    attachment: activeRpcPayload?.params ? (activeRpcPayload?.params[0]?.attachment as string) : undefined,
  });
  ...
  useEffect(() => {
    if (walletCreationError || didTokenError) {
      ...
      logger.error(
        'magic_auth_generate_id_token Page - Wallet hydrating or creation error',
