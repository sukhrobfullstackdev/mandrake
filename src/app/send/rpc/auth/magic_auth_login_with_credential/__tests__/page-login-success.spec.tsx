import Page from '@app/send/rpc/auth/magic_auth_login_with_credential/page';
...
jest.mock('next/navigation', () => ({
  ...
  usePathname: jest.fn(() => '/send/rpc/auth/magic_auth_login_with_credential'),
}));
...
jest.mock('@hooks/data/embedded/redirect-login', () => ({
  useRedirectLoginQuery: jest.fn().mockImplementation(() => ({})),
}));

describe('magic_auth_login_with_credential page', () => {
  function executeRequest() {
    AtomicRpcPayloadService.setActiveRpcPayload({
      jsonrpc: '2.0',
      method: 'magic_auth_login_with_credential',
      id: 'my_id',
      params: ['?magic_credential=dsadsadsa' as any],
    });
  ...
  it('hydrate session and use get or create eth wallet should be called', () => {
    ...
    expect(resolveJsonRpcResponse).toHaveBeenCalledWith({
      payload: {
        ...
        jsonrpc: '2.0',
        method: 'magic_auth_login_with_credential',
        params: ['?magic_credential=dsadsadsa'],
      },
      sdkMetadata: {},
      analyticsProperties: {
        api_key: 'magicApiKey',
        ...
        env: 'test-testnet',
        eventOrigin: '',
        json_rpc_method: 'magic_auth_login_with_credential',
        rpcUrl: 'mainnet',
        sdk: 'SdkMissing',
