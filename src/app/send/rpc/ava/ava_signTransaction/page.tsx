...
import { AvaxLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function AvaxSignTransactionPage() {
  ...
  const doAvaxSignTransactionForUser = async () => {
    ...
    try {
      ...
      const avaxBridge = (await createBridge(getAccount, ethNetwork, ext)).ledgerBridge as AvaxLedgerBridge;
      const res = await avaxBridge.signTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'ava_signTrasaction Page - Error hydrating or creating multichain wallet',
