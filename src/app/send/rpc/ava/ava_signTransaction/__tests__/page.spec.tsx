import Page from '@app/send/rpc/ava/ava_signTransaction/page';
...
const mockBridge = { signTransaction: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { signTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated avax signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
      expect(mockResolve).toHaveBeenCalledWith('signature');
      expect(mockReject).not.toHaveBeenCalled();
    });
  });

  it('user logged in: should call rejectActiveRpcRequest if avax bridge throws error', async () => {
    (DkmsService.reconstructSecret as jest.Mock).mockResolvedValue('decryptedPrivateKey');
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockBridgeError }));
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
