import Page from '@app/send/rpc/ava/ava_getAccount/page';
...
const mockBridge = { getAccount: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { getAccount: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated avax signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.getAccount).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if avax bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.getAccount).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
