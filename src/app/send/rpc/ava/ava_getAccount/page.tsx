...
export default function AvaxGetAccountPage() {
  ...
  const doAvaxGetAccountForUser = async () => {
    ...
    try {
      ...
      const res = await avaxBridge.getAccount(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'ava_getAccount Page - Error hydrating or creating multichain wallet',
