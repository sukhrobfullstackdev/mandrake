...
export default function IconGetBalancePage() {
  ...
  const doIcxGetBalanceForUser = async () => {
    ...
    try {
      ...
      const res = await iconBridge.getBalance(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'icx_getBalance Page - Error hydrating or creating multichain wallet',
