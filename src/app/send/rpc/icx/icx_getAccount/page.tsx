...
export default function IconGetAccountPage() {
  ...
  const doIconGetAccountForUser = async () => {
    ...
    try {
      ...
      const res = await iconBridge.getAccount(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'icx_getAccount Page - Error hydrating or creating multichain wallet',
