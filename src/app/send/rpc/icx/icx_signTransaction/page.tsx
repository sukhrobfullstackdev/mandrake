...
import { IconLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function IconSignTransactionPage() {
  ...
  const doIconSignTransactionForUser = async () => {
    ...
    try {
      ...
      const iconBridge = (await createBridge(getAccount, ethNetwork, ext)).ledgerBridge as IconLedgerBridge;
      const res = await iconBridge.signTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'icx_signTransaction Page - Error hydrating or creating multichain wallet',
