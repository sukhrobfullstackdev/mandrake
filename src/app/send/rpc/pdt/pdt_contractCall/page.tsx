...
export default function PolkadotContractCallPage() {
  ...
  const doPolkadotContractCallForUser = async () => {
    ...
    try {
      ...
      const res = await polkadotBridge.contractCall(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'pdt_contactCall Page - Error hydrating or creating multichain wallet',
