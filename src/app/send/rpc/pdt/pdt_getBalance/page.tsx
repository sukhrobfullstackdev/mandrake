...
export default function PolkadotGetBalancePage() {
  ...
  const doPolkadotGetBalanceForUser = async () => {
    ...
    try {
      ...
      const res = await polkadotBridge.getBalance(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'pdt_getBalance Page - Error hydrating or creating multichain wallet',
