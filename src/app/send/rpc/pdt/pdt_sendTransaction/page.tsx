...
export default function PolkadotSendTransactionPage() {
  ...
  const doPolkadotSendTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await polkadotBridge.sendTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doPolkadotSendTransactionForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'pdt_sendTransaction Page - Error hydrating or creating multichain wallet',
