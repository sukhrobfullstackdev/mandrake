...
export default function PolkadotSignRawPage() {
  ...
  const doPolkadotSignRawForUser = async () => {
    ...
    try {
      ...
      const res = await polkadotBridge.signRaw(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error('pdt_signRaw Page - Error hydrating or creating multichain wallet', multichainWalletHydrationError);
    }
