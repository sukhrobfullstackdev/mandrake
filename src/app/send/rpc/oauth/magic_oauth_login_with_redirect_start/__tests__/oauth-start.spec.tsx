...
jest.mock('next/navigation', () => ({
  ...
  usePathname: jest.fn(() => '/send/rpc/auth/magic_oauth_login_with_redirect_start'),
}));
...
describe('OAuthLoginWithRedirect Start', () => {
  ...
  it('resolves the RPC request successfullly', async () => {
    ...
    expect(mockResolveJsonRpcResponse).toHaveBeenCalled();
  });

  it('resolves with the correct oauthAuthoriationURI', async () => {
    (useClientConfigAccessAllowlists as jest.Mock).mockReturnValue({
      domain: ['https://test.com'],
      redirectUrl: ['https://test.com'],
    });
    ...
    expect(mockResolveJsonRpcResponse).toHaveBeenCalledWith({
      oauthAuthoriationURI: `https://accounts.google.com/o/oauth2/v2/auth?client_id=abcdef&redirect_uri=https%3A%2F%2Ftest.com&state=${encodedState}&scope=openid%20email%20profile&code_challenge=0987poiu&code_challenge_method=S256&response_type=code&access_type=offline`,
  ...
  it('throws an error if redirectURI is missing from params', async () => {
    (useClientConfigAccessAllowlists as jest.Mock).mockReturnValue({
      domain: ['https://test.com'],
      redirectUrl: ['https://test.com'],
    });
    ...
    await act(async () => {
      await setupInvalid({ paramsOverride: { redirectURI: '' } });
    });
  ...
  it('resolves the RPC request successfully for a different provider (Facebook)', async () => {
    (useClientConfigAccessAllowlists as jest.Mock).mockReturnValue({
      domain: ['https://test.com'],
      redirectUrl: ['https://test.com'],
    });
