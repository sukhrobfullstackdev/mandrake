...
import { CfxLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function CfxSendTransactionPage() {
  ...
  const { walletInfoData, credentialsData, multichainWalletHydrationError } = useHydrateOrCreateMultichainWallet({
    enabled: !!authUserId && !!authUserSessionToken,
    walletType: WalletType.CONFLUX,
  });
  const cfxGetAccount = () => walletInfoData?.publicAddress || '';
  const rejectActiveRpcRequest = useRejectActiveRpcRequest();
  const resolveActiveRpcRequest = useResolveActiveRpcRequest();

  const doCfxSendTransactionForUser = async () => {
    ...
    try {
      ...
      const cfxBridge = (await createBridge(cfxGetAccount, ethNetwork, ext)).ledgerBridge as CfxLedgerBridge;
      const res = await cfxBridge.sendTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'cfx_sendTransaction Page - Error hydrating or creating multichain wallet',
