import Page from '@app/send/rpc/cfx/cfx_sendTransaction/page';
...
const mockCfxBridge = { sendTransaction: jest.fn().mockResolvedValue('signature') };
const mockCfxBridgeError = { sendTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  beforeEach(() => {
    ...
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockCfxBridge }));
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockCfxBridge.sendTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    (DkmsService.reconstructSecret as jest.Mock).mockResolvedValue('decryptedPrivateKey');
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockCfxBridgeError }));
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockCfxBridgeError.sendTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
