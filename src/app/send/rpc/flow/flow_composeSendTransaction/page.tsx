...
export default function FlowComposeSendTransactionPage() {
  ...
  const doFlowComposeSendFlowForUser = async () => {
    ...
    try {
      ...
      const res = await flowBridge.composeSendFlow(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'flow_composeSendTransaction Page - Error hydrating or creating multichain wallet',
