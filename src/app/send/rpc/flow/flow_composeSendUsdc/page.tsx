...
export default function FlowComposeSendUsdcPage() {
  ...
  const doFlowComposeSendUsdcForUser = async () => {
    ...
    try {
      ...
      const res = await flowBridge.composeSendUsdc(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'flow_composeSendUsdc Page - Error hydrating or creating multichain wallet',
