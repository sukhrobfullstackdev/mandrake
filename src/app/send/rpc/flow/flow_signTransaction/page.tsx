...
export default function FlowSignTransactionPage() {
  ...
  const doFlowSignTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await flowBridge.signTransaction(activeRpcPayload, pk);
      resolveActiveRpcRequest(res);
    } catch (err) {
      if (err instanceof Error) {
        logger.error(`Flow Sign Transaction Error: ${err.message}`, { error: err });
      }
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'flow_signTransaction Page - Error hydrating or creating multichain wallet',
