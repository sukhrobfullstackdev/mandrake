...
export default function AptosGetAccountPage() {
  ...
  const doAptosGetAccountForUser = async () => {
    ...
    try {
      ...
      const res = await aptosBridge.getAccount(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'aptos_getAccount Page - Error hydrating or creating multichain wallet',
