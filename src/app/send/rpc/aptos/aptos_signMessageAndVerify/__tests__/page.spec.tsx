import Page from '@app/send/rpc/aptos/aptos_signMessageAndVerify/page';
...
const mockBridge = { signMessageAndVerify: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { signMessageAndVerify: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.signMessageAndVerify).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.signMessageAndVerify).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
