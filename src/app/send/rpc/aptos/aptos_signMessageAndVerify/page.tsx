...
export default function AptosSignMessageAndVerifyPage() {
  ...
  const doAptosSignMessageAndVerifyForUser = async () => {
    ...
    try {
      ...
      const res = await aptosBridge.signMessageAndVerify(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doAptosSignMessageAndVerifyForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'algod_signMessageAndVerify Page - Error hydrating or creating multichain wallet',
