import Page from '@app/send/rpc/aptos/aptos_getAccountInfo/page';
...
const mockBridge = { getAccountInfo: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { getAccountInfo: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.getAccountInfo).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.getAccountInfo).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
