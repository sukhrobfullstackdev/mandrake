...
export default function AptosGetAccountInfoPage() {
  ...
  const doAptosGetAccountInfoForUser = async () => {
    ...
    try {
      ...
      const res = await aptosBridge.getAccountInfo(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'aptos_getAccountInfo Page - Error hydrating or creating multichain wallet',
