...
export default function AptosSignMessagePage() {
  ...
  const doAptosSignMessageForUser = async () => {
    ...
    try {
      ...
      const res = await aptosBridge.signMessage(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'aptos_signMessage Page - Error hydrating or creating multichain wallet',
