...
export default function AptosSignAndSubmitTransactionPage() {
  ...
  const doAptosSignAndSubmitTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await aptosBridge.signAndSubmitTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doAptosSignAndSubmitTransactionForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'aptos_signAndSubmitTransaction Page - Error hydrating or creating multichain wallet',
