import Page from '@app/send/rpc/aptos/aptos_signAndSubmitTransaction/page';
...
const mockBridge = { signAndSubmitTransaction: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { signAndSubmitTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.signAndSubmitTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.signAndSubmitTransaction).toHaveBeenCalledWith(
