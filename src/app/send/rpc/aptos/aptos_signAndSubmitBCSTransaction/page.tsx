...
export default function AptosSignAndSubmitBCSTransactionPage() {
  ...
  const doAptosSignAndSubmitBCSTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await aptosBridge.signAndSubmitBCSTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doAptosSignAndSubmitBCSTransactionForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'aptos_signAndSubmitBCSTransaction Page - Error hydrating or creating multichain wallet',
