import Page from '@app/send/rpc/aptos/aptos_signAndSubmitBCSTransaction/page';
...
const mockBridge = { signAndSubmitBCSTransaction: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { signAndSubmitBCSTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.signAndSubmitBCSTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.signAndSubmitBCSTransaction).toHaveBeenCalledWith(
