...
import { BtcLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function BtcSignTransactionPage() {
  ...
  const btcGetAccount = () => walletInfoData?.publicAddress || '';
  const rejectActiveRpcRequest = useRejectActiveRpcRequest();
  const resolveActiveRpcRequest = useResolveActiveRpcRequest();

  const doBtcSignTransactionForUser = async () => {
    ...
    try {
      ...
      const btcBridge = (await createBridge(btcGetAccount, ethNetwork, ext)).ledgerBridge as BtcLedgerBridge;
      const res = await btcBridge.signTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'btc_signTransaction Page - Error hydrating or creating multichain wallet',
