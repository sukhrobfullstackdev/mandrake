import Page from '@app/send/rpc/btc/btc_signTransaction/page';
...
const mockBtcBridge = { signTransaction: jest.fn().mockResolvedValue('signature') };
const mockBtcBridgeError = { signTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  beforeEach(() => {
    ...
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockBtcBridge }));
    (AtomicRpcPayloadService.getActiveRpcPayload as jest.Mock).mockReturnValue(mockActiveRpcPayload);
  });

  afterEach(() => jest.clearAllMocks());

  it('user logged in: should call resolveActiveRpcRequest with successfuly generated btc signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBtcBridge.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
      expect(mockResolve).toHaveBeenCalledWith('signature');
      expect(mockReject).not.toHaveBeenCalled();
    });
  });

  it('user logged in: should call rejectActiveRpcRequest if btc bridge throws error', async () => {
    (DkmsService.reconstructSecret as jest.Mock).mockResolvedValue('decryptedPrivateKey');
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockBtcBridgeError }));
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBtcBridgeError.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
