...
export default function NearGetPublicKeyPage() {
  ...
  const doNearGetPublicKeyForUser = async () => {
    ...
    try {
      ...
      const res = await nearBridge.getPublicKey(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'near_getPublicKey Page - Error hydrating or creating multichain wallet',
