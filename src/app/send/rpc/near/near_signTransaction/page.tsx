...
import { NearLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function NearSignTransactionPage() {
  ...
  const doNearSignTransactionForUser = async () => {
    ...
    try {
      ...
      const nearBridge = (await createBridge(getAccount, ethNetwork, ext)).ledgerBridge as NearLedgerBridge;
      const res = await nearBridge.signTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'near_signTransaction Page - Error hydrating or creating multichain wallet',
