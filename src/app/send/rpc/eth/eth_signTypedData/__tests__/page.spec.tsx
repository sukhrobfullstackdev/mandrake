import Page from '@app/send/rpc/eth/eth_signTypedData/page';
...
const mockActiveRpcPayload = { params: ['message', '0x123'] };
...
describe('Page Component', () => {
  ...
  it('expected signer: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    (Web3Service.signTypedDataV1 as jest.Mock).mockResolvedValue('signature');

    await waitFor(async () => {
      await executeRequest();

      expect(Web3Service.compareAddresses).toHaveBeenCalledWith([
        mockActiveRpcPayload.params[1],
        mockEthWalletInfo.publicAddress,
      ]);
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(
        expect.anything(),
        mockEthWalletInfo.encryptedPrivateAddress,
      );
      expect(Web3Service.signTypedDataV1).toHaveBeenCalledWith(mockActiveRpcPayload.params[0], 'decryptedPrivateKey');
  ...
  it('unexpected signer: should call rejectActiveRpcRequest with user denied signing error', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(Web3Service.signTypedDataV1).not.toHaveBeenCalled();
