...
import { TerraLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function TerraGetPublicKeyPage() {
  ...
  const doTerraGetPublicKeyForUser = async () => {
    ...
    try {
      ...
      const terraBridge = (await createBridge(getAccount, ethNetwork, ext)).ledgerBridge as TerraLedgerBridge;
      const res = await terraBridge.getPublicKey(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'terra_getPublicKey Page - Error hydrating or creating multichain wallet',
