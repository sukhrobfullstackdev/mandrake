import Page from '@app/send/rpc/terra/terra_getPublicKey/page';
...
const mockBridge = { getPublicKey: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { getPublicKey: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.getPublicKey).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.getPublicKey).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
