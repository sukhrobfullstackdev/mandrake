...
export default function TerraSignPage() {
  ...
  const doTerraSignForUser = async () => {
    ...
    try {
      ...
      const res = await terraBridge.sign(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error('terra_sign Page - Error hydrating or creating multichain wallet', multichainWalletHydrationError);
    }
