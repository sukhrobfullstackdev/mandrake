...
export default function SolSignTransactionPage() {
  ...
  const doSolSendTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await solanaBridge.signTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'sol_signTransaction Page - Error hydrating or creating multichain wallet',
