import Page from '@app/send/rpc/sol/sol_signTransaction/page';
...
const mockSolanaBridge = { signTransaction: jest.fn().mockResolvedValue('signature') };
const mockSolanaBridgeError = { signTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated solana signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockSolanaBridge.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if solana bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockSolanaBridgeError.signTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
