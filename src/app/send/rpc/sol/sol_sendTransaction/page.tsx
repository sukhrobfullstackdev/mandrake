...
export default function SolSendTransactionPage() {
  ...
  const doSolSendTransactionForUser = async () => {
    ...
    try {
      ...
      const res = await solanaBridge.sendTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'sol_sendTransaction Page - Error hydrating or creating multichain wallet',
