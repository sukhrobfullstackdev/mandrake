...
export default function SolSignMessagePage() {
  ...
  const doSolSignMessageForUser = async () => {
    ...
    try {
      ...
      const res = await solanaBridge.signMessage(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'sol_signMessage Page - Error hydrating or creating multichain wallet',
