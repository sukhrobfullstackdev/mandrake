import Page from '@app/send/rpc/sol/sol_partialSignTransaction/page';
...
const mockSolanaBridge = { partialSignTransaction: jest.fn().mockResolvedValue('signature') };
const mockSolanaBridgeError = { partialSignTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated solana signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockSolanaBridge.partialSignTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if solana bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockSolanaBridgeError.partialSignTransaction).toHaveBeenCalledWith(
