import Page from '@app/send/rpc/cos/cos_sendTokens/page';
...
const mockCosmosBridge = { sendTokens: jest.fn().mockResolvedValue('signature') };
const mockCosmosBridgeError = { sendTokens: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated cosmos signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockCosmosBridge.sendTokens).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if cosmos bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockCosmosBridgeError.sendTokens).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
