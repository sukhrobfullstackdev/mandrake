...
export default function CosSendTokensPage() {
  ...
  const doCosSendTokensForUser = async () => {
    ...
    try {
      ...
      const res = await cosmosBridge.sendTokens(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'cos_sendTokens Page - Error hydrating or creating multichain wallet',
