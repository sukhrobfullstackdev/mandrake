...
export default function CosChangeAddressPage() {
  ...
  const doCosChangeAddressForUser = async () => {
    ...
    try {
      ...
      const res = await cosmosBridge.changeAddress(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'cos_changeAddress Page - Error hydrating or creating multichain wallet',
