import Page from '@app/send/rpc/cos/cos_changeAddress/page';
...
const mockCosmosBridge = { changeAddress: jest.fn().mockResolvedValue('signature') };
const mockCosmosBridgeError = { changeAddress: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated cosmos signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockCosmosBridge.changeAddress).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if cosmos bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockCosmosBridgeError.changeAddress).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
