...
export default function CosSignPage() {
  ...
  const doCosSignForUser = async () => {
    ...
    try {
      ...
      const res = await cosmosBridge.sign(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error('cos_sign Page - Error hydrating or creating multichain wallet', multichainWalletHydrationError);
    }
