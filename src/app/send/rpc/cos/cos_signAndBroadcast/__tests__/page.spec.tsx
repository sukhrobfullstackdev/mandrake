import Page from '@app/send/rpc/cos/cos_signAndBroadcast/page';
...
const mockCosmosBridge = { signAndBroadcast: jest.fn().mockResolvedValue('signature') };
const mockCosmosBridgeError = { signAndBroadcast: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  beforeEach(() => {
    ...
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockCosmosBridge }));
    (AtomicRpcPayloadService.getActiveRpcPayload as jest.Mock).mockReturnValue(mockActiveRpcPayload);
  });
  afterEach(() => jest.clearAllMocks());

  it('user logged in: should call resolveActiveRpcRequest with successfuly generated cosmos signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockCosmosBridge.signAndBroadcast).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
      expect(mockResolve).toHaveBeenCalledWith('signature');
      expect(mockReject).not.toHaveBeenCalled();
    });
  });

  it('user logged in: should call rejectActiveRpcRequest if cosmos bridge throws error', async () => {
    (DkmsService.reconstructSecret as jest.Mock).mockResolvedValue('decryptedPrivateKey');
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockCosmosBridgeError }));
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockCosmosBridgeError.signAndBroadcast).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
