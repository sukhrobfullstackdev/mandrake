...
import { CosmosLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function CosSignAndBroadcastPage() {
  ...
  const cosGetAccount = () => walletInfoData?.publicAddress || '';

  const rejectActiveRpcRequest = useRejectActiveRpcRequest();
  const resolveActiveRpcRequest = useResolveActiveRpcRequest();

  const doCosSignAndBroadcastForUser = async () => {
    ...
    try {
      ...
      const cosmosBridge = (await createBridge(cosGetAccount, ethNetwork, ext)).ledgerBridge as CosmosLedgerBridge;
      const res = await cosmosBridge.signAndBroadcast(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'cos_signAndBroadcast Page - Error hydrating or creating multichain wallet',
