import Page from '@app/send/rpc/taquito/taquito_sign/page';
...
const mockBridge = { sign: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { sign: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.sign).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.sign).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
