...
export default function TaquitoSignPage() {
  ...
  const doTaquitoSignForUser = async () => {
    ...
    try {
      ...
      const res = await taquitoBridge.sign(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error('taquito_sign Page - Error hydrating or creating multichain wallet', multichainWalletHydrationError);
    }
