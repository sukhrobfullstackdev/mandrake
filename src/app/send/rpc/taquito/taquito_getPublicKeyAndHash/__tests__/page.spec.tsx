import Page from '@app/send/rpc/taquito/taquito_getPublicKeyAndHash/page';
...
const mockBridge = { getPublicKeyAndHash: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { getPublicKeyAndHash: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.getPublicKeyAndHash).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.getPublicKeyAndHash).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
