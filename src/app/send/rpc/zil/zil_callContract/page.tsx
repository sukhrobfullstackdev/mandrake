...
export default function ZilCallContractPage() {
  ...
  const doZilliqaCallContractForUser = async () => {
    ...
    try {
      ...
      const res = await zilliqaBridge.callContract(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'zil_callContract Page - Error hydrating or creating multichain wallet',
