import Page from '@app/send/rpc/zil/zil_deployContract/page';
...
const mockBridge = { deployContract: jest.fn().mockResolvedValue('signature') };
const mockBridgeError = { deployContract: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  it('user logged in: should call resolveActiveRpcRequest with successfuly generated signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockBridge.deployContract).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
  ...
  it('user logged in: should call rejectActiveRpcRequest if bridge throws error', async () => {
    ...
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockBridgeError.deployContract).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
