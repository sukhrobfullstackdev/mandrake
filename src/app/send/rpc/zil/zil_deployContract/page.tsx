...
import { ZilliqaLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function ZilDeployContractPage() {
  ...
  const doZilliqaDeployContractForUser = async () => {
    ...
    try {
      ...
      const zilliqaBridge = (await createBridge(getAccount, ethNetwork, ext)).ledgerBridge as ZilliqaLedgerBridge;
      const res = await zilliqaBridge.deployContract(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doZilliqaDeployContractForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'zil_deployContract Page - Error hydrating or creating multichain wallet',
