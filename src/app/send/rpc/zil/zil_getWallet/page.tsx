...
export default function ZilGetWalletPage() {
  ...
  const doZilliqaGetWalletForUser = async () => {
    ...
    try {
      ...
      const res = await zilliqaBridge.getWallet(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'zil_getWallet Page - Error hydrating or creating multichain wallet',
