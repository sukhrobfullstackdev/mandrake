...
import { SuiLedgerBridge } from '@custom-types/ledger-bridge';
...
export default function SuiSignAndSendTransactionPage() {
  ...
  const suiGetAccount = () => walletInfoData?.publicAddress || '';
  const rejectActiveRpcRequest = useRejectActiveRpcRequest();
  const resolveActiveRpcRequest = useResolveActiveRpcRequest();

  const doSuiSignAndSendTransactionForUser = async () => {
    ...
    try {
      ...
      const suiBridge = (await createBridge(suiGetAccount, ethNetwork, ext)).ledgerBridge as SuiLedgerBridge;
      const res = await suiBridge.signAndSendTransaction(activeRpcPayload, pk);
  ...
  useEffect(() => {
    ...
    doSuiSignAndSendTransactionForUser();
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'sui_signAndSendTransaction Page - Error hydrating or creating multichain wallet',
