import Page from '@app/send/rpc/sui/sui_signAndSendTransaction/page';
...
const mockSuiBridge = { signAndSendTransaction: jest.fn().mockResolvedValue('signature') };
const mockSuiBridgeError = { signAndSendTransaction: jest.fn().mockRejectedValue('error') };
...
describe('Page Component', () => {
  ...
  beforeEach(() => {
    ...
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockSuiBridge }));
    (AtomicRpcPayloadService.getActiveRpcPayload as jest.Mock).mockReturnValue(mockActiveRpcPayload);
  });

  afterEach(() => jest.clearAllMocks());

  it('user logged in: should call resolveActiveRpcRequest with successfuly generated sui signature', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(mockSuiBridge.signAndSendTransaction).toHaveBeenCalledWith(mockActiveRpcPayload, 'decryptedPrivateKey');
      expect(mockResolve).toHaveBeenCalledWith('signature');
      expect(mockReject).not.toHaveBeenCalled();
    });
  });

  it('user logged in: should call rejectActiveRpcRequest if sui bridge throws error', async () => {
    (DkmsService.reconstructSecret as jest.Mock).mockResolvedValue('decryptedPrivateKey');
    (createBridge as jest.Mock).mockResolvedValue(Promise.resolve({ ledgerBridge: mockSuiBridgeError }));
    await waitFor(async () => {
      await executeRequest();
      expect(DkmsService.reconstructSecret).toHaveBeenCalledWith(expect.anything(), mockWallet.encryptedPrivateAddress);
      expect(mockSuiBridgeError.signAndSendTransaction).toHaveBeenCalledWith(
        mockActiveRpcPayload,
        'decryptedPrivateKey',
      );
