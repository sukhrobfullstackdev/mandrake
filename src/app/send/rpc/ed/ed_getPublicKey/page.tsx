...
export default function EdGetPublicKeyPage() {
  ...
  const doEdGetPublicKeyForUser = async () => {
    ...
    try {
      ...
      const res = await edBridge.getPublicKey(activeRpcPayload, pk);
  ...
  useEffect(() => {
    if (multichainWalletHydrationError) {
      ...
      logger.error(
        'ed_getPublicKey Page - Error hydrating or creating multichain wallet',
